...
class BarcodetoPostCodeTransformer{
    transformToPostCode(barcode) {
        ...
        if(flag === false){
            return {error: `输入的条码不能有：和|以外的字符`, code: ``};
        }

        if(flag1 === false){
            return {error: `输入的条码必须头部和尾部有| 和 |`, code: ``};
        }

        if(flag2 === false){
            return {error: `输入的条码digit的数目必须是6或10`, code: ``};
        }
        ...
        if (flag && flag1 && flag2) {
            ...
            if(codeArray === 'error'){
                return {error: `cd is wrong`, code: ``};

            }else {
                return {error: ``, code: getPostCode(codeArray)};
            }
        }
    }
